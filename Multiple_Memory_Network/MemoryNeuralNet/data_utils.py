...
def load_dialog_task(data_dir, candid_dic, isOOV):
    ...
    print(" Also this is the correct one")

    test_data = list()
    for test_file in glob.glob(os.path.join(data_dir,'test/test_data*.txt')) :
        #print("loading file name : {} ".format(test_file))
        #london = input(" waiting input ")
        test_file_data = get_dialogs(test_file,candid_dic)
