...
class MultipleMemoryModel(object):
    ...
    def __init__(self, data_dir, model_dir, converse_later, OOV, memory_size, random_state, batch_size, learning_rate, epsilon, max_grad_norm, evaluation_interval, hops, epochs, embedding_size,session,description):
    ...
    def print_results(self,cost,accuracy,description) :
        ...
        print("===========================> {} cost      = {} ".format(description,str(cost)))
    ...
    def display_results(self,train_cost_list,val_cost_list,epoch_list) :
        ...
        plt.xlabel('#Epochs')
        plt.ylabel('Cost')
        plt.title(self.description + '_performance.png')
    ...
    def train(self):
        ...
        for t in range(1, self.epochs + 1):
            ...
            self.display_results(train_cost_list=train_cost_list,val_cost_list,epoch_list=epoch_list)

            if t % self.evaluation_interval == 0:
                train_preds = self.batch_predict(trainS, trainQ, n_train)
                val_preds = self.batch_predict(valS, valQ, n_val)
                train_acc = metrics.accuracy_score(np.array(train_preds), trainA)
                val_acc = metrics.accuracy_score(np.array(val_preds), valA)
                '''print('-----------------------')
                print('Epoch', t)
                ...
                if val_cost < best_validation_cost :
        ...
        plt.plot(epoch_list,val_cost_list,'r',label='validation cost')
        ...
        performance_dir = "performance_charts/"
        if not os.path.exists(performance_dir) :
            os.makedirs(performance_dir)

        file_location = os.path.join(performance_dir,save_image_name)
