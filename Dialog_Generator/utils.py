    ...
    for file_name in glob.glob('template_folder/*.xlsx') :
        ...
        for index,row in df.iterrows() :
            ...
            if not pd.isnull(label) :
                if label not in template_dictionary.keys() :
                    template_sentences = set()
                else :
                    template_sentences = template_dictionary[label]
            
                
                if not pd.isnull(text) :
                    template_sentences.add(text)
            
                template_dictionary[label] = template_sentences
        del df
