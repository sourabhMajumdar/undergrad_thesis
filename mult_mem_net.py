...
if __name__ == '__main__':
    ...
    if not os.path.exists("start_mem_network/"):
        os.makedirs("start_mem_network")

    if not os.path.exists("transaction_history_mem_network/"):
        os.makedirs("transaction_history_mem_network")

    if not os.path.exists("account_balance_mem_network/"):
        os.makedirs("account_balance_mem_network/")
    ...
    with g1.as_default() :
        main_mem_network = chatBot(data_dir="data/start_data/", 
    ...
    with g2.as_default() :
        transaction_history_mem_network = chatBot(data_dir="data/transaction_history_data/", 
                    model_dir="transaction_history_mem_network/",    
                    task_id=1,  
                    isInteractive=FLAGS.interactive,    
                    OOV=FLAGS.OOV,
                    memory_size=FLAGS.memory_size,
    ...
    #loan = input("transaction_history_mem_network created successfully !!")
    
    with g3.as_default() :
        account_balance_mem_network = chatBot(data_dir="data/account_balance_data/", 
                    model_dir="account_balance_mem_network/",    
                    task_id=1,  
                    isInteractive=FLAGS.interactive,    
                    OOV=FLAGS.OOV,
                    memory_size=FLAGS.memory_size,
    ...
    #balance = input("account_balance_mem_network created successfully !!")
    
    with g4.as_default() :
        transaction_mem_network = chatBot(data_dir="data/transaction_data/", 
    ...
    if FLAGS.train:
        ...
        time.sleep(10)
        with g2.as_default() :
            transaction_history_mem_network.train()
        time.sleep(10)
        with g3.as_default():
            account_balance_mem_network.train()
        time.sleep(10)
        with g4.as_default() :
        ...
        transaction_history_mem_network.test()
        account_balance_mem_network.test()
        transaction_mem_network.test()
    #chatbot.converse()

    network_dict = {"main" : [main_mem_network.converse_2,g1] , "transaction_history" : [transaction_history_mem_network.converse_2,g2], "account_balance" : [account_balance_mem_network.converse_2,g3], "transaction" : [transaction_mem_network.converse_2,g4]}

    story = list()
    nid = 1
    network_converse = main_mem_network.converse_2
    network_graph = g1
    user_utterance = "<SILENCE>"
    while True :

        with network_graph.as_default() :
            bot_utterance , story, nid = network_converse(story,user_utterance,nid)
            if "mem_call" in bot_utterance :
                words = bot_utterance.strip().split(":")
                network_graph = network_dict[words[1]][1]
                network_converse = network_dict[words[1]][0]
                continue
            print(bot_utterance)
